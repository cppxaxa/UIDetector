...
def compo_detection_img_bytes(image_bytes, uied_params,
                    resize_by_height=800, classifier=None, show=False, wai_key=0):

    # *** Step 1 *** pre-processing: read img -> get binary map
    org, grey = pre.read_img_bytes(image_bytes, resize_by_height)
    binary = pre.binarization(org, grad_min=int(uied_params['min-grad']))
    ...
    uicompos += nesting_inspection(org, grey, uicompos, ffl_block=uied_params['ffl-block'])
    Compo.compos_update(uicompos, org.shape)
    result_img = draw.draw_bounding_box(org, uicompos, line=1, show=show, name='merged compo', write_path=None, wait_key=wai_key)

    # *** Step 7 *** save detection result
    Compo.compos_update(uicompos, org.shape)
    
    img_shape = uicompos[0].image_shape
    ...
    print("[Compo Detection Completed]")
